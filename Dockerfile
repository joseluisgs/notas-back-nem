...
# Luego, creamos a imagen: docker build -t joseluisgs/notas-back-nem .  (oye que lo del punto no está mal, es que es así :))
# Luego: docker run -p 49160:8000 -d joseluisgs/notas-back-nem de esta manera exponemos los puertos. en nuestra maquina es el 49160, que se mapea al 8000 del docker
# Luego: docker ps
